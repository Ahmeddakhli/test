...
async (req: express.Request, res: express.Response): Promise<void> => {
    ...
    if (!img_file_name||!img_width||!img_height) {
        ...
        const image_path: string = path.join(__dirname,'../../../public/oraginimages',`${img_file_name}.jpg`);
        if (!fs.existsSync(image_path)) {         // If there is no such file
          res.status(404).send( `${img_file_name} does not exist .check the file name`);
        } else {
          const resizedImage: string = path.join(__dirname, '../../../public/resizedimages',`${img_file_name}-${img_width}-${img_height}-thumb.jpg`);
  
          if (fs.existsSync(resizedImage)) {   // Return resizedImage, If we already have resized image 
            ...
            res.sendFile(
              path.join(__dirname,'../../../public/resizedimages',`${img_file_name}-${img_width}-${img_height}-thumb.jpg`));
          }
        }
      }
});
