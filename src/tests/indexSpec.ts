...
describe('test main response', () => {it('To verify express app is working or not', async () => {
  ...
  it('It should return 404  when there is no file', async () => {
    const fileName = 'fjor';
    const response = await request.get( `/api/image_api?filename=${fileName}&width=200&height=200`);
...
//It should resize the image
  it('It should resize the image', async () => {
    ...
    const filePath = path.join(__dirname,'../../public/resizedimages', 'fjord-200-200-thumb.jpg' );
